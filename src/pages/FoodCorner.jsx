            ...
            <div className="flex flex-wrap items-center justify-center gap-5  py-5">
                {
                   !input ? Categroy.map((item,index)=>(
                        <div key={index} onClick={()=>filterData(item.name)} className=" bg-white shadow-2xl rounded-2xl w-20 h-20 flex flex-col items-center justify-center hover:bg-orange-200 transition-all duration-300">
                            {item.icon}
                            {item.name}
                        </div>
                    )) : ""
            ...
            <div className="h-full w-full flex flex-wrap gap-10 justify-center py-10">
              ...
              {cat.length>1 ? 
                ...
                cat.map((item,index)=>(
                    ...
                    <button onClick={()=>{dispatch(AddItem({id:index,name:item.food_name,image:item.food_image,price:item.price,qty:item.food_quantity}));toast.success("item added succesfully!")}} className="bg-orange-200 text-lg font-medium w-full h-10 rounded hover:bg-orange-400 transition-all  ">Add To Cart</button>
